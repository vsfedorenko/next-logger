...
3. **Классификатор логов Next** (`patches/next.ts`, рантайм) — проверяет каждый
   вызов `console.*`: если первый аргумент похож на вывод Next.js, строка
   тегируется `next.js`; иначе — `console`. Похожей на вывод Next.js считается
   строка, которая после снятия ANSI-кодов либо несёт символ-маркер
   (`▲`, `✓`, `⚠`, `●`, `✗`, …) — возможно, за информационным префиксом `ℹ`,
   либо совпадает с формой запросного лога dev-сервера
   (`GET /path 200 in 716ms …`), либо начинается со скобочного префикса
   компонента (`[MDX] …`, `[console 6:12 PM] …`).
