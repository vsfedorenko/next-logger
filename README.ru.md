...
**ссылайтесь на него по имени** из сериализуемого конфига.
...
Смежные хелперы: `getReporter` / `hasReporter` / `removeReporter`
(доступ к реестру), `resolveReporters` (спека → живой репортер).
...
Пресеты — просто данные: положите их в общий внутренний пакет, храните стеки
под окружения (`"production"`, `"development"`, `"ci"`) в версионном
контроле и переключайте окружение, меняя одну строку.

## Плагин-система

Поставляйте переиспользуемые настройки логирования как npm-пакеты:
регистрируйте именованные фабрики репортеров и **пресеты** конфигурации из
JavaScript, ссылайтесь на них **по имени** из сериализуемого конфига.
Функции не пересекают границу «сборка → рантайм» как JSON — имена пересекают.
...
- `defineReporter(name, factory)` — регистрирует фабрику репортеров.
  Повторная регистрация заменяет. Фабрика получает сериализуемые `options`
  из конфига и возвращает consola-репортер; секреты читайте внутри фабрики
  из окружения (та же политика, что у встроенных репортеров).
- `definePreset(name, preset)` — регистрирует набор конфигурации (`backend`,
  `backendOptions`, опции `consola`, `reporters`). Явные ключи в
  `withLogger({...})` выигрывают у пресетных.
- Репортеры резолвятся на `init()`; неизвестное имя кидает ошибку **на
  старте** со списком зарегистрированных — расхождение конфига падает громко,
  логи никогда не пропадают молча.
- Репортеры цепляются только к consola-логгерам; другие бэкенды
  (pino/winston) приносят свои назначении и игнорируют спеки репортеров.
- Встроенная фабрика `"json"` зарегистрирована всегда; сетевые репортеры
  (datadog/otlp/sentry/pino) живут на subpath-энтрипоинтах — оборачивайте
  их через `defineReporter()`, чтобы главный бандл оставался tree-shakeable.

Хелперы реестра: `hasReporter` / `removeReporter` / `getReporter`,
`hasPreset` / `removePreset` / `getPreset` (все экспортированы из корня).
